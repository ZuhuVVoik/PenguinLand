...
public class IKAnimation : MonoBehaviour
{
    ...
    void OnAnimatorIK() // метод подобен Update, но используется для программных анимаций
    {
        if (interact)
        {
            if (weight < 1) weight += 0.01f; // т.к. вес меняется от 0 до 1, а изначально мы задали 0, то для плавного перехода руки от исходной позиции достаточно плавно изменять вес ik анимации.
    ...
    public void StopInteraction() { interact = false; }
